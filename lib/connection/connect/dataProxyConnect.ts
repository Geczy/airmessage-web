import DataProxy from "lib/connection/dataProxy";

import * as CloseFrame from "./webSocketCloseEventCodes";
import * as NHT from "./nht";
import ByteBuffer from "bytebuffer";
import { getAuth, getIdToken } from "firebase/auth";
import { getInstallationID } from "lib/util/installationUtils";
...
import TaskQueue from "lib/util/taskQueue";
...
export default class DataProxyConnect extends DataProxy {
  ...
  start(): void {
    ...
    getIdToken(getAuth().currentUser!)
      .then((idToken: string) => {
        //Building the URL
        const url = new URL(process.env.NEXT_PUBLIC_connectHostname as string);
