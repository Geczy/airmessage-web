...
import { modifierUpdateEmitter } from "lib/connection/connectionManager";
import { getNotificationUtils } from "lib/interface/notification/notificationUtils";
import {
  playSoundMessageIn,
  playSoundNotification,
  playSoundTapback,
} from "lib/util/soundUtils";
  ...
  const applyUpdateMessages = useCallback(
    async (newItems: ConversationItem[]) => {
      ...
      if (unlinkedSortedConversationItems.size > 0) {
        ...
        for (const [chatGUID, conversationItems] of Array.from(
          unlinkedSortedConversationItems.entries()
        )) {
